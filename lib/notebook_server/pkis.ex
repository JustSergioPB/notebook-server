defmodule NotebookServer.PKIs do
  ...
  def create_certificate(user_id, org_id) do
    {private_certificate, user_certificate} = KeyPair.generate()
    ...
    |> case do
      {:ok, user_certificate} ->
        create_private_certificate(org_id, user_certificate.id, private_certificate)
        {:ok, user_certificate}
  ...
  def rotate_certificate(user_id, org_id, old_user_certificate) do
    {private_certificate, user_certificate} = KeyPair.generate()
    ...
    |> case do
      {:ok,
       %{old_user_certificate: old_user_certificate, new_user_certificate: new_user_certificate}} ->
        create_private_certificate(org_id, new_user_certificate.id, private_certificate)
        delete_private_certificate(org_id, old_user_certificate.id)
  ...
  def revoke_certificate(user_certificate) do
    user_certificate
    ...
    |> case do
      {:ok, user_certificate} ->
        delete_private_certificate(user_certificate.org_id, user_certificate.id)
        {:ok, user_certificate}
  ...
  defp create_private_certificate(org_id, user_certificate_id, private_certificate) do
    encryptedCertificate = PrivateKey.encrypt(private_certificate)
    File.mkdir_p!("./#{org_id}/certificates")
    File.write!("./#{org_id}/certificates/#{user_certificate_id}.bin", encryptedCertificate)
  end

  defp delete_private_certificate(org_id, user_certificate_id) do
    File.rm!("./#{org_id}/certificates/#{user_certificate_id}.bin")
  end
